...
public class Admin extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (taskid==100){
			...
			String deptdesc="";
			try {
				st = con.prepareStatement("SELECT DEPT_ID,DEPT_NAME from DEPARTMENT where DEPT_ID=(SELECT DEPARTMENT_ID from JOB_OPPORTUNITY WHERE OPPORTUNITY_ID=?)");
			...
			try {
				st = con.prepareStatement("SELECT EMPLOYEE_ID,NAME from EMPLOYEE where DEPT_ID=?");
				st.setInt(1, deptid);
				ResultSet resultset=st.executeQuery();
				if(depthead == 0)
			...
			try{
				...
				if(matl.matches())
				{
					//String sql="INSERT INTO EMPLOYEE(POSITION_ID) VALUES(?,?,?,?,?,?,?,?,?)";
					String sql="UPDATE EMPLOYEE SET POSITION_ID=?,SUPERVISOR=?,SALARY=?,DEPT_ID=?,QUAL_ID=?,GENDER=?,NAME=?,ROLE=? WHERE EMPLOYEE_ID=?";			
		...
		else if(taskid==2)
		{
			...
			int dept_head=Integer.parseInt(request.getParameter("depthead"));
			
			String sql="INSERT INTO DEPARTMENT(DEPT_NAME,DEPT_HEAD) VALUES(?,?)";
			try {
				PreparedStatement statement = con.prepareStatement(sql);
				statement.setString(1,dept_name);
				statement.setInt(2,dept_head);
				statement.executeUpdate();
		...
		else if(taskid==3)
		{
			...
			String sql="INSERT INTO QUALIFICATION(QUAL_DESC) VALUES(?)";
			try {
				PreparedStatement statement = con.prepareStatement(sql);
		...
		else if(taskid==4)
		{
			...
			String sql="INSERT INTO POSITION(POS_DESC)  VALUES(?)";
			try {
				PreparedStatement statement = con.prepareStatement(sql);
		...
		else if(taskid==5)
		{
			...
			String sql="INSERT INTO JOB_OPPORTUNITY(OPPORTUNITY_ID,DEPARTMENT_ID,POSITION_ID,NO_OF_VACANCIES,LAST_DATE) VALUES(?,?,?,?,?)";
			try {
				PreparedStatement statement = con.prepareStatement(sql);
			...
			String sql="UPDATE DEPARTMENT SET DEPT_HEAD=? where DEPT_ID=?";
			try{
			PreparedStatement statement = con.prepareStatement(sql);
			...
			String posdesc="";
			try {
				st = con.prepareStatement("SELECT POSITION_ID,POS_DESC from POSITION where POSITION_ID=(SELECT POSITION_ID from JOB_OPPORTUNITY WHERE OPPORTUNITY_ID=?)");
